...
lazy_static! {
    ...
    static ref SPOTIFY_TRACK_REGEX: Regex = Regex::new(
        r"^(https?://)?(open\.spotify\.com|spotify)/track/([a-zA-Z0-9]+)(\?.*)?$"
    ).unwrap();
    static ref SPOTIFY_PLAYLIST_REGEX: Regex = Regex::new(
        r"^(https?://)?(open\.spotify\.com|spotify)/playlist/([a-zA-Z0-9]+)(\?.*)?$"
    ).unwrap();
    static ref SPOTIFY_ALBUM_REGEX: Regex = Regex::new(
        r"^(https?://)?(open\.spotify\.com|spotify)/album/([a-zA-Z0-9]+)(\?.*)?$"
    ).unwrap();
}
...
impl SpotifyApi {
    ...
    pub fn extract_track_id(url: &str) -> Option<String> {
        SPOTIFY_TRACK_REGEX.captures(url)
    ...
    pub fn extract_playlist_id(url: &str) -> Option<String> {
        SPOTIFY_PLAYLIST_REGEX.captures(url)
            .and_then(|cap| cap.get(3))
            .map(|m| m.as_str().to_string())
    }
    ...
    pub fn extract_album_id(url: &str) -> Option<String> {
        SPOTIFY_ALBUM_REGEX.captures(url)
            .and_then(|cap| cap.get(3))
            .map(|m| m.as_str().to_string())
    }
    ...
    async fn get_access_token() -> SpotifyResult<String> {
        ...
        let response = HTTP_CLIENT.post("https://accounts.spotify.com/api/token")
            .header(header::AUTHORIZATION, auth_header)
            .form(&params)
            .send()
            .await
            .map_err(|e| MusicError::ExternalApiError(format!("Failed to request Spotify token: {}", e)))?;
        
        if !response.status().is_success() {
            let status = response.status();
            let text = response.text().await.unwrap_or_else(|_| "Cannot read response".to_string());
            return Err(MusicError::ExternalApiError(format!("Spotify API error: {} - {}", status, text)));
        }
        
        // Parse token response
        let token_response = response.json::<SpotifyToken>().await
            .map_err(|e| MusicError::ExternalApiError(format!("Failed to parse Spotify token: {}", e)))?;
    ...
    pub async fn get_track(track_id: &str) -> SpotifyResult<SpotifyTrack> {
        ...
        let response = HTTP_CLIENT.get(&url)
            ...
            .send()
            .await
            .map_err(|e| MusicError::ExternalApiError(format!("Failed to request Spotify track: {}", e)))?;
        ...
        let track_data: serde_json::Value = response.json().await
            .map_err(|e| MusicError::ExternalApiError(format!("Failed to parse Spotify track data: {}", e)))?;
    ...
    pub async fn get_playlist_tracks(playlist_id: &str) -> SpotifyResult<Vec<SpotifyTrack>> {
        ...
        let mut url = format!("https://api.spotify.com/v1/playlists/{}/tracks?limit=50", playlist_id);
        ...
        loop {
            let response = HTTP_CLIENT.get(&url)
                ...
                .send()
                .await
                .map_err(|e| MusicError::ExternalApiError(format!("Failed to request Spotify playlist: {}", e)))?;
            ...
            let playlist_data: serde_json::Value = response.json().await
                .map_err(|e| MusicError::ExternalApiError(format!("Failed to parse Spotify playlist data: {}", e)))?;
    ...
    pub async fn get_album_tracks(album_id: &str) -> SpotifyResult<Vec<SpotifyTrack>> {
        ...
        let album_response = HTTP_CLIENT.get(&album_url)
            .header(header::AUTHORIZATION, format!("Bearer {}", token))
            .send()
            .await
            .map_err(|e| MusicError::ExternalApiError(format!("Failed to request Spotify album: {}", e)))?;
        
        if !album_response.status().is_success() {
            let status = album_response.status();
            let text = album_response.text().await.unwrap_or_else(|_| "Cannot read response".to_string());
            return Err(MusicError::ExternalApiError(format!("Spotify API error: {} - {}", status, text)));
        }
        ...
        loop {
            let response = HTTP_CLIENT.get(&url)
                ...
                .send()
                .await
                .map_err(|e| MusicError::ExternalApiError(format!("Failed to request Spotify album tracks: {}", e)))?;
            ...
            let tracks_data: serde_json::Value = response.json().await
                .map_err(|e| MusicError::ExternalApiError(format!("Failed to parse Spotify album tracks data: {}", e)))?;
