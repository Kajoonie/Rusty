...
impl AudioSource {
    ...
    pub async fn get_related_songs(url: &str) -> AudioSourceResult<Vec<TrackMetadata>> {
        ...
        let video_id = if let Some(captures) = YOUTUBE_REGEX.captures(url) {
            ...
            return Err(MusicError::AudioSourceError("Not a valid YouTube URL".to_string()));
        };

        let video_id = match video_id {
            Some(id) => id,
            None => return Err(MusicError::AudioSourceError("Could not extract video ID".to_string())),
        };
        ...
        if related_songs.is_empty() {
            ...
            let search_output = Command::new("yt-dlp")
                .args([
                    ...
                    "--no-download",
                    "--default-search", "ytsearch5",  // Search for 5 videos
                    &search_term
                ])
                .output()
                .map_err(|e| MusicError::AudioSourceError(format!("Failed to search for related videos: {}", e)))?;
            ...
            for line in search_str.lines() {
                if let Ok(video_json) = serde_json::from_str::<serde_json::Value>(line) {
                    ...
                    let video_url = video_json["webpage_url"]
                        .as_str()
                        .map(|s| s.to_string());
                    ...
                    let duration = video_json["duration"]
                        .as_f64()
                        .map(Duration::from_secs_f64);
                    
                    let thumbnail = video_json["thumbnail"]
                        .as_str()
                        .map(|s| s.to_string());
    ...
    pub async fn from_spotify_url(url: &str) -> AudioSourceResult<(Input, TrackMetadata)> {
        ...
        if let Some(track_id) = SpotifyApi::extract_track_id(url) {
            ...
            if tracks.is_empty() {
                return Err(MusicError::AudioSourceError("Spotify playlist is empty".to_string()));
            }
            ...
            if tracks.is_empty() {
                return Err(MusicError::AudioSourceError("Spotify album is empty".to_string()));
            }
    ...
    pub async fn from_spotify_track(track: SpotifyTrack) -> AudioSourceResult<(Input, TrackMetadata)> {
