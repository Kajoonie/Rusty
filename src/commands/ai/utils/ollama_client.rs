...
impl OllamaClient {
    ...
    fn get_conversation_history(&self, user: &User) -> Vec<ChatMessage> {
        ...
        self.convo_map.entry(user.clone()).or_insert_with(|| {
            debug!("Initializing new conversation history for user {}", user.name);
            Mutex::new(vec![])
        });
    
        let user_convo = self.convo_map.get(user).unwrap();
        let messages = user_convo.lock().unwrap();
        debug!("Retrieved {} messages from history for user {}", messages.len(), user.name);
        messages.clone()
    }
    ...
    pub async fn chat(&self, user: &User, message: &str) -> Result<ChatMessageResponse, OllamaError> {
        ...
        match self.client.clone().send_chat_messages_with_history(
            &mut chat_history,
            ...
            Ok(response) => {
                debug!("Successfully received response from Ollama for user {}", user.name);
                Ok(response)
            }
            Err(e) => {
                error!("Failed to get response from Ollama for user {}: {}", user.name, e);
                Err(e)
            }
        }
    }
